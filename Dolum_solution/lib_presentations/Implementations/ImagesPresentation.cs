...
public class ImagesPresentation : IImagesPresentation
{
    private IImagesComunication? iComunication = null;
    public ImagesPresentation(IImagesComunication iComunication)
    {
        this.iComunication = iComunication;
    }
    public async Task<List<Images>> GetList()
    {
        ...
        list = JsonConverter.ConvertToObject<List<Images>>(
            JsonConverter.ConvertToString(answer["Entities"]));
        return list;
    }
    public async Task<List<Images>> Search(Images entity, string type)
    {
        var list = new List<Images>();
        var data = new Dictionary<string, object>();
        data["Entity"] = entity;
    ...
    public async Task<Images> Save(Images entity)
    {
    ...
    public async Task<Images> Delete(Images entity)
    {
        if (entity.Id == 0 || !entity.Validate())
        {
            throw new Exception("lbWrongData");
    ...
    public async Task<Images> Modify(Images entity)
    {
        if (entity.Id == 0 || !entity.Validate())
        {
            throw new Exception("lbWrongData");
